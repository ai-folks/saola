...
class Interface:
    ...
    def _execute(self):
        ...
        if ((not self.safety_checks or not self.convo.safety_checks) and self.convo.console.no_safety_confirmation()) \
           or self.convo.console.safety_confirmation(f"[red1] Execute {self.name} code? [/red1]"):
...
class InterfacedConvo(Convo):
    def __init__(self, model, *, console=None, interfaces=None, safety_checks=True):
        super().__init__(model, console=console)
    ...
    def stream_answer(self, *handlers):
        ...
        while True:
            ...
            if output:
                self.console.display_interface_output(output)
...
class LocalFileWriteInterface(Interface):
    name = "FILE_WRITE"
