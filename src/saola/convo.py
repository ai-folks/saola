...
from saola.console import LocalShellConsole

class Convo:
    def __init__(self, model=None, console=None):
        self.bubbles = []
        self.checkpoints = []
        self.model = model() if isinstance(model, type) else model
        self.console = console or LocalShellConsole()
    ...
    def loop(self):
        with self:
            while True:
                self.console.display_user_header()
                self.user << self.console.get_user_input()
                self.console.display_assistant_header()
                self.stream_answer_to_end(self.console.append_to_assistant_output)
